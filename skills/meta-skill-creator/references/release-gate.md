...
3. 产品定义：目标用户、最终产物、3 分钟可见结果和一票否决项明确。
...
8. 输出：输出评测覆盖模糊输入、不完整输入、高风险输入、真实材料输入、陌生领域研究、多模态/工具路线和最终产物复查。
9. 基线：同一提示词分别用本技能包和不用本技能包跑；如果本技能包不明显优于基线，回到设计阶段。
10. 回归：新版本与旧版本对比，不扩大误触发、不猜领域、不忽略本地工具、不丢有用资产。
11. 资产：至少一个非文档资产或结构数据存在，并绑定具体失败模式。
12. 安全与原创：不得打包凭证、私密数据、不安全自动化、照搬示例、照搬提示词模板或照搬销售话术。
13. 可见测试与漂移：至少保留一次用户可见或干净会话运行记录；依赖宿主工具、MCP、原生选择界面或生成产物的技能包，必须证明对应路线，或标为 partial / blocked；干净会话提示必须保留用户原始模糊意图，不能追加隐藏验收提示；失败要回写到评测或参考。
...
十四项门禁全部通过后，才可以声称包已就绪。`quick_validate.py` 或结构脚本通过只证明结构，不证明产品质量。系统级替换或首次公开分发，还需要独立干净会话基准、真实产物证据、人工验收和闭环写回决策。
